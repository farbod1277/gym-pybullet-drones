...
if __name__ == "__main__":
    ...
    filename = os.path.join(dirname, "expert_trajectories_slanted_" + str(num_trajs) + ".pkl")
...
#########
    ...
    bc_logger = logger.configure(os.path.join(dirname,"./BC/" + str(num_trajs)))
    bc_trainer = bc.BC(
    ...
    n_epochs = 100
    for i in range(n_epochs, 1000000000000, n_epochs):
        bc_logger = logger.configure(os.path.join(dirname,"./BC/" + str(num_trajs) + "/" + str(i)))
        bc_trainer._logger = bc_logger
        bc_trainer.logger
        bc_trainer.train(n_epochs=n_epochs)
        bc_trainer.save_policy(os.path.join(dirname, "./BC/" + str(num_trajs) + "/bc_model_" + str(i) + "_on_" + str(num_trajs) + "_trajs.pt"))