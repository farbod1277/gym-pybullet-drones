...
if __name__ == "__main__":
    ...
    gail_logger = logger.configure(os.path.join(dirname,"./GAIL_PPO/" + str(num_trajs)))
    ...
    for i in range(n_epochs, 10000000000000, n_epochs):
        gail_logger = logger.configure(os.path.join(dirname,"./GAIL_PPO/" + str(num_trajs) + "/" + str(i)))
        gail_trainer._logger = gail_logger
        gail_trainer.train(total_timesteps=n_epochs)
        gail_trainer.gen_algo.save(os.path.join(dirname, "./GAIL_PPO/" + str(num_trajs) + "/gail_model_" + str(i) + "_on_" + str(num_trajs) + "_trajs.zip"))
